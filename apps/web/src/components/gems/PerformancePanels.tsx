...
import type { GemBaselineComparison, GemComponentEdge, GemPerformance, GemScoreEdge } from '@/lib/types';

/**
 * The "has this scanner actually worked?" panels.
 *
 * Lifted out of the page so they can be rendered in a test against an
 * older API payload. Every one of `scoreEdge`, `componentEdges` and
 * `baseline` is optional in GemPerformance — web and API deploy
 * separately, so a new client meets an old response on every release, and
 * this page is the one a required-but-missing field has already taken out
 * once. A component that only a page can construct cannot be checked for
 * that.
 */
interface PerformancePanelProps {
  data: GemPerformance | null;
  loading: boolean;
  error: string | null;
}
 ...
 */
export function GemPerformancePanel({ data, loading, error }: PerformancePanelProps) {
 ...
 */
export function MarketBaselinePanel({ baseline }: { baseline?: GemBaselineComparison }) {
 ...
 */
export function ScoreEdgePanel({ edge }: { edge?: GemScoreEdge }) {
 ...
 */
export function ComponentEdgePanel({ edges }: { edges?: GemComponentEdge[] }) {
 ...
 */
function bandName(
  verdict: NonNullable<GemScoreEdge['verdict']>,
  end: 'low' | 'high',
): string {
...
function ComponentVerdict({ edge }: { edge: GemComponentEdge }) {
  if (edge.degenerate) {
    return (
